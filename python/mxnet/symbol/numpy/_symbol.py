...
from ...base import c_array, c_array_buf, c_str, c_str_array, c_handle_array
...
class _Symbol(Symbol):
        ...
        if num_outputs == 1:
            ...
            if not self._output_is_list:
                if isinstance(key, integer_types):
                    self = _npi.slice(self, key, key+1)
                    return _npi.reshape(self, (-3, -4))
                    ...
                    for index in key:
                        if isinstance(index, py_slice):
                            ...
                            new_shape += (-3,)
                    new_shape += (-4,)
                    self = _npi.slice(self, begin, end, step)
                    self = _npi.reshape(self, new_shape)
                    return self
    ...
    def __setitem__(self, key, value):
        num_outputs = len(self)
        # slice the list of nodes in the symbol
        if num_outputs > 1:
            raise NotImplementedError
        elif num_outputs == 1:
            if self._output_is_list:
                ...
                if isinstance(key, integer_types):
                    if isinstance(value, numeric_types):
                        return _npi.slice_assign_scalar(self, scalar=value, begin=key, end=key+1)
        raise NotImplementedError
...
def load_json_string(json_str):
        ...
        s = _Symbol(handle)
        s._output_is_list = output_is_list
        return s
